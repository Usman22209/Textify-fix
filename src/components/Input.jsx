...
export default function Input() {
  const { dark, selectedNote, setSelectedNote } = useContext(AppContext);
  ...
  const showAlert = useCallback((message) => {
    ...
    if (selectedNote) {
      ...
      // Optional: Clear the selected note after loading
  ...
  return (
    <div
      className={`min-h-[calc(100vh-70px)]  mt-5 ${
    ...
    >
      ...
      <div
        className={`sm:w-4/5 mx-auto p-6 ${
      ...
      >
        ...
        <div className="mt-4 text-sm">
          Characters: {text.trim().length} | Words:{" "}
          {text.trim() ? text.trim().split(/\s+/).length : 0} | Read Time:{" "}
          {(text.trim().length / 300).toFixed(2)} mins
        </div>
